...
const handler: Request = {
    ...
    exec: async (u: User, data: any, payload: any, context?: ModuleContext) => {
        ...
        // Get game module (e.g., fyrnvale)
        // The game module should have initialized crafting
        const gameModule = context.getModule('fyrnvale'); // TODO: Make this configurable
        if (!gameModule) {
        ...
        const fyrnvaleModule = context.getModule('fyrnvale');
        if (!fyrnvaleModule) {
            u.error("Game module not found");
            return;
        }

        const allRecipes = fyrnvaleModule.data?.recipes || {};
