            ...
            for (const item of loot) {
                if (this.useMemory) {
                    ...
                    const result = await this.context.db.execute(
                        `INSERT INTO fyrnvale_dropped_loot
        ...
        try {
            ...
            if (this.useMemory) {
                ...
                const [rows] = await this.context.db.execute(
                    'SELECT * FROM fyrnvale_dropped_loot WHERE id = ? AND character_id = ?',
                    [lootId, characterId]
                );
                ...
                if (new Date(loot.expires_at) < new Date()) {
                    this.context.logger.warn(`Loot ${lootId} has expired`);
                    await this.context.db.execute('DELETE FROM fyrnvale_dropped_loot WHERE id = ?', [lootId]);
                    return false;
                }
            }
    ...
    async getLootInZone(characterId: number, zone: string): Promise<DroppedItem[]> {
        try {
            if (this.useMemory) {
                ...
                // Database storage
                const [rows] = await this.context.db.execute(
                    'SELECT * FROM fyrnvale_dropped_loot WHERE character_id = ? AND zone = ? AND expires_at > NOW()',
                    [characterId, zone]
                );
    ...
    async cleanupExpiredLoot(): Promise<number> {
        try {
            if (this.useMemory) {
                ...
                // Database storage
                const result = await this.context.db.execute(
                    'DELETE FROM fyrnvale_dropped_loot WHERE expires_at < NOW()'
                );
