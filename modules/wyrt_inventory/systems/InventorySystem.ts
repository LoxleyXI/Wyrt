...
export class InventorySystem {
        ...
        try {
            ...
            if (existing && existing.quantity < this.maxStackSize) {
                ...
                await this.context.db.execute(
                    'UPDATE fyrnvale_inventory SET quantity = ? WHERE id = ?',
            ...
            // Insert new item
            await this.context.db.execute(
                'INSERT INTO fyrnvale_inventory (character_id, item_id, quantity, slot, equipped) VALUES (?, ?, ?, ?, FALSE)',
    ...
    async removeItem(characterId: number, itemId: string, quantity: number): Promise<boolean> {
        try {
            ...
            if (item.quantity === quantity) {
                // Remove entire stack
                await this.context.db.execute(
                    'DELETE FROM fyrnvale_inventory WHERE id = ?',
                    [item.id]
                );
            } else {
                // Decrease quantity
                await this.context.db.execute(
                    'UPDATE fyrnvale_inventory SET quantity = quantity - ? WHERE id = ?',
                    [quantity, item.id]
                );
            }
    ...
    async getInventory(characterId: number): Promise<InventoryItem[]> {
        try {
            const [rows] = await this.context.db.execute(
                'SELECT * FROM fyrnvale_inventory WHERE character_id = ? ORDER BY slot ASC',
                [characterId]
            );
    ...
    async getItem(characterId: number, itemId: string): Promise<InventoryItem | null> {
        try {
            const [rows] = await this.context.db.execute(
                'SELECT * FROM fyrnvale_inventory WHERE character_id = ? AND item_id = ? LIMIT 1',
                [characterId, itemId]
            );
    ...
    async moveItem(characterId: number, fromSlot: number, toSlot: number): Promise<boolean> {
        try {
            ...
            const [fromRows] = await this.context.db.execute(
                'SELECT * FROM fyrnvale_inventory WHERE character_id = ? AND slot = ?',
            ...
            if (toItem) {
                // Swap items
                await this.context.db.execute(
                    'UPDATE fyrnvale_inventory SET slot = ? WHERE id = ?',
                    [toSlot, fromItem.id]
                );
    ...
    async hasSpace(characterId: number, requiredSlots: number = 1): Promise<boolean> {
        try {
            const [rows] = await this.context.db.execute(
                'SELECT COUNT(*) as count FROM fyrnvale_inventory WHERE character_id = ?',
                [characterId]
            );
    ...
    async findEmptySlot(characterId: number): Promise<number | null> {
        try {
            const [rows] = await this.context.db.execute(
                'SELECT slot FROM fyrnvale_inventory WHERE character_id = ? AND slot IS NOT NULL ORDER BY slot ASC',
                [characterId]
            );
    ...
    async setSlot(characterId: number, itemId: string, slot: number): Promise<boolean> {
        try {
            const result = await this.context.db.execute(
                'UPDATE fyrnvale_inventory SET slot = ? WHERE character_id = ? AND item_id = ?',
