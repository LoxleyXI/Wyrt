...
export class InventorySystem {
    ...
    async addItem(characterId: number, itemId: string, quantity: number, slot: number | null = null): Promise<boolean> {
        try {
            ...
            if (existing && existing.quantity < this.maxStackSize) {
                ...
                await this.context.prisma.fyrnvale_inventory.update({
            ...
            // Insert new item
            await this.context.prisma.fyrnvale_inventory.create({
                data: {
    ...
    async removeItem(characterId: number, itemId: string, quantity: number): Promise<boolean> {
        try {
            ...
            if (item.quantity === quantity) {
                // Remove entire stack
                await this.context.prisma.fyrnvale_inventory.delete({
                    where: { id: item.id }
                });
            } else {
                // Decrease quantity
    ...
    async getInventory(characterId: number): Promise<InventoryItem[]> {
        try {
            const rows = await this.context.prisma.fyrnvale_inventory.findMany({
    ...
    async getItem(characterId: number, itemId: string): Promise<InventoryItem | null> {
        try {
            const row = await this.context.prisma.fyrnvale_inventory.findFirst({
                where: {
                    character_id: characterId,
                    item_id: itemId
                }
    ...
    async moveItem(characterId: number, fromSlot: number, toSlot: number): Promise<boolean> {
        try {
            ...
            const fromItem = await this.context.prisma.fyrnvale_inventory.findFirst({
                where: {
                    character_id: characterId,
            ...
            const toItem = await this.context.prisma.fyrnvale_inventory.findFirst({
                where: {
                    character_id: characterId,
    ...
    async hasSpace(characterId: number, requiredSlots: number = 1): Promise<boolean> {
        try {
            const count = await this.context.prisma.fyrnvale_inventory.count({
                where: { character_id: characterId }
            });
    ...
    async setSlot(characterId: number, itemId: string, slot: number): Promise<boolean> {
        try {
            const result = await this.context.prisma.fyrnvale_inventory.updateMany({
                where: {
                    character_id: characterId,
                    item_id: itemId
                },
