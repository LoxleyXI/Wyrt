...
export class EquipmentSystem {
        ...
        try {
            ...
            if (!equipment) {
                ...
                await this.context.db.execute(
                    'INSERT INTO fyrnvale_equipment (character_id) VALUES (?)',
                    [characterId]
                );
            }
            ...
            // Update equipment
            await this.context.db.execute(
                `UPDATE fyrnvale_equipment SET ${slot} = ? WHERE character_id = ?`,
                [itemId, characterId]
            );
    ...
    async unequip(characterId: number, slot: string): Promise<boolean> {
        try {
            ...
            // Update equipment
            await this.context.db.execute(
                `UPDATE fyrnvale_equipment SET ${slot} = NULL WHERE character_id = ?`,
                [characterId]
            );
    ...
    async getEquipment(characterId: number): Promise<EquipmentSet | null> {
        try {
            const [rows] = await this.context.db.execute(
                'SELECT * FROM fyrnvale_equipment WHERE character_id = ?',
                [characterId]
            );
