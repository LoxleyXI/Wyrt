...
export class EquipmentSystem {
    ...
    async equip(characterId: number, itemId: string, slot: string): Promise<boolean> {
        try {
            ...
            if (!equipment) {
                ...
                await this.context.prisma.fyrnvale_equipment.create({
                    data: {
            ...
            await this.context.prisma.fyrnvale_equipment.update({
    ...
    async getEquipment(characterId: number): Promise<EquipmentSet | null> {
        try {
            const equipment = await this.context.prisma.fyrnvale_equipment.findUnique({
                where: { character_id: characterId }
            });
